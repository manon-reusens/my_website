import type { Talk, FilterOptions } from '../types';
...
export const talksData: Talk[] = [
  ...
  },
  {
    ...
    links: {
      event: "https://www.vlaanderen.be/statistiek-vlaanderen/dag-van-het-netwerk-statistiek-vlaanderen-op-21-maart-2024",
...
const DEFAULT_EXCLUDED_TALKS: string[] = [];
...
function normalizeTalksOpts(opts: FilterOptions = {}): NormalizedFilterOptions {
  return {
    excludeIds: new Set([
      ...(opts.excludeIds || []),
      ...(opts.useDefaultExclusions ? DEFAULT_EXCLUDED_TALKS : [])
    ]),
    excludeTypes: new Set(opts.excludeTypes || []),
    includeHidden: !!opts.includeHidden,
    excludePredicate: typeof opts.excludePredicate === 'function' ? opts.excludePredicate : null,
    useDefaultExclusions: opts.useDefaultExclusions,
    excludeTalkIds: opts.excludeTalkIds || []
  };
}

function filterTalks(arr: Talk[], opts: FilterOptions = {}): Talk[] {
  const o = normalizeTalksOpts(opts);
  return arr.filter(t => {
    if (!o.includeHidden && t.hiddenFromTalks) return false;
    if (o.excludeIds.has(String(t.id))) return false;
    if (o.excludeTypes.size && o.excludeTypes.has(t.type)) return false;
...
export const TalksAPI = {
  getAll: (opts?: FilterOptions) => filterTalks([...talksData], opts),
  getByType: (type: string, opts?: FilterOptions) => filterTalks(
    talksData.filter(t => t.type === type),
    opts
  ),
  getByYear: (year: string, opts?: FilterOptions) => filterTalks(
    talksData.filter(t => t.date && t.date.startsWith(year)),
    opts
  ),
  getRecent: (count: number = 5, opts?: FilterOptions) => filterTalks([...talksData], opts)
    .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
    .slice(0, count),
  getSortedByDate: (opts?: FilterOptions) => filterTalks([...talksData], opts)
    .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
  getKeynotes: (opts?: FilterOptions) => filterTalks(
    talksData.filter(t => t.type === "Keynote"),
    opts
  ),
  getKeynotesAndInvited: (opts?: FilterOptions) => filterTalks(
    talksData.filter(t => t.type === "Keynote" || t.type === "Invited Talk"),
    opts
  ),
  groupByYear: (opts?: FilterOptions) => {
    const grouped: Record<string, Talk[]> = {};
    filterTalks([...talksData], opts).forEach(talk => {
