...
export const PublicationsAPI = {
  ...
  getByType: (type: string) => publicationsData.filter(p => p.type === type),
  getByYear: (year: number) => publicationsData.filter(p => p.year === year),
  ...
  getSortedByDate: () => {
    return [...publicationsData].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }
};
